...
ParseTreeNode *parse_assignment() {
    ...
    if(current_token < token_length && myTokens[current_token].type==TOKEN_OPERATOR && myTokens[current_token].value[0] == '=')
...
ParseTreeNode *parse_term() {
    ...
    ParseTreeNode *factor_node = parse_power();
    if (!factor_node) {
